...
def parse_args():
    ...
    parser.add_argument(
        "--input-dir", type=str, default="data/engineered", help="Input dizini (default: data/engineered)"
    )

    parser.add_argument(
        "--output-dir", type=str, default="data/selected", help="Output dizini (default: data/selected)"
    )

    parser.add_argument("--n-features", type=int, default=50, help="Seçilecek feature sayısı (default: 50)")
    ...
    parser.add_argument("--save-plots", action="store_true", help="Feature importance grafiklerini kaydet")
...
def save_data(
    train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame, output_dir: Path, selected_features: list
):
    """
...
def main():
    ...
    train_selected, val_selected, test_selected, selector, selection_info = select_features_train_val_test(
        train_df=train_df, val_df=val_df, test_df=test_df, n_features=args.n_features, method=args.method
    )
