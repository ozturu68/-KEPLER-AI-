...
def parse_args():
    ...
    parser.add_argument("--input-dir", type=str, default="data/scaled", help="Input dizini (default: data/scaled)")

    parser.add_argument(
        "--output-dir", type=str, default="data/engineered", help="Output dizini (default: data/engineered)"
    )

    parser.add_argument(
        "--polynomial", action="store_true", help="Polynomial features oluştur (çok fazla feature oluşturur!)"
    )

    parser.add_argument("--poly-degree", type=int, default=2, help="Polynomial degree (default: 2)")

    parser.add_argument("--no-planetary", action="store_true", help="Planetary features oluşturma")

    parser.add_argument("--no-interactions", action="store_true", help="Interaction features oluşturma")
...
def show_new_features(original_cols: list, engineered_cols: list):
    ...
    if new_features:
        ...
        planetary = [
            ...
            if any(key in f for key in ["planet", "orbital", "stellar", "habitable", "transit", "teq", "snr"])
        ]
...
def main():
    ...
    engineer = ExoplanetFeatureEngineer(poly_degree=args.poly_degree, interaction_only=False, include_bias=False)
