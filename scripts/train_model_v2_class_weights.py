...
def train_strategy(strategy_name, class_weights, X_train, y_train, X_val, y_val, X_test, y_test):
    ...
    if class_weights == "Balanced":
        model = CatBoostModel(
            iterations=1000, learning_rate=0.03, depth=6, l2_leaf_reg=3, auto_class_weights="Balanced", verbose=False
        )
    elif class_weights is None:
        model = CatBoostModel(iterations=1000, learning_rate=0.03, depth=6, l2_leaf_reg=3, verbose=False)
    else:
        model = CatBoostModel(
            iterations=1000, learning_rate=0.03, depth=6, l2_leaf_reg=3, class_weights=class_weights, verbose=False
        )
...
def main():
    ...
    for strategy_name, class_weights in strategies:
        ...
        try:
            result = train_strategy(strategy_name, class_weights, X_train, y_train, X_val, y_val, X_test, y_test)
