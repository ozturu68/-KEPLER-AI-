...
def analyze_predictions(model, X, y, dataset_name="Dataset"):
    ...
    logger.info(f"   Düşük confidence (<{low_conf_threshold}): {low_conf_count} ({low_conf_count/len(max_proba)*100:.2f}%)")
...
def analyze_errors(y_true, y_pred, top_n=10):
    ...
    if y_true_arr.shape != y_pred.shape:
        raise ValueError(
            f"y_true ve y_pred shape'leri uyuşmuyor! "
            f"y_true: {y_true_arr.shape}, y_pred: {y_pred.shape}"
        )
...
def compare_datasets(train_metrics, val_metrics, test_metrics):
    ...
    logger.info("-" * 50)
    logger.info(f"{'Accuracy':<15s} {train_metrics['acc']:>10.4f} {val_metrics['acc']:>10.4f} {test_metrics['acc']:>10.4f}")
    logger.info(f"{'F1 Score':<15s} {train_metrics['f1']:>10.4f} {val_metrics['f1']:>10.4f} {test_metrics['f1']:>10.4f}")
