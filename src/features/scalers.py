...
class FeatureScaler:
    ...
    def fit(self, df: pd.DataFrame, exclude_cols: list[str] | None = None) -> "FeatureScaler":
        """
        Scaler'ı fit et.
    ...
    def fit_transform(self, df: pd.DataFrame, exclude_cols: list[str] | None = None) -> pd.DataFrame:
        """
...
def compare_scalers(df: pd.DataFrame, sample_features: list[str] | None = None) -> dict:
    """
