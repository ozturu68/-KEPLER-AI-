...
from src.core import TARGET_COLUMN, RANDOM_STATE, FeatureSelectionError
...
class FeatureSelector:
    ...
    def remove_high_correlation_features(
        ...
        if high_corr_features:
            ...
            for feat in high_corr_features[:5]:
                correlated_with = upper_triangle[feat][
                    upper_triangle[feat] > self.correlation_threshold
                ].index.tolist()
                if correlated_with:
                    logger.debug(
                        f"    - {feat} → {correlated_with[0]}: "
                        f"{upper_triangle[feat][correlated_with[0]]:.3f}"
                    )
        else:
    ...
    def get_feature_importance(
        ...
        if remaining_nan > 0 or remaining_inf > 0:
            raise FeatureSelectionError(
                f"Temizleme başarısız! NaN: {remaining_nan}, INF: {remaining_inf}"
            )
    ...
    def select_top_features(
        ...
        if n_features is not None:
            ...
            logger.info(
                f"  ✓ {len(selected)} feature seçildi "
                f"(importance >= {importance_threshold})"
            )
            
        else:
            raise FeatureSelectionError(
                "n_features veya importance_threshold belirtilmeli!"
            )
    ...
    def select_features(
        ...
        low_var_features = self.remove_low_variance_features(
            df_numerical,
        ...
        high_corr_features = self.remove_high_correlation_features(
            df_filtered,
            exclude_cols=[target_col]
        )
        ...
        if after_filtering > n_features:
            ...
            logger.info(
                f"  ✓ Tüm feature'lar seçildi ({len(selected_features)} < {n_features})"
            )
    ...
    def transform(self, df: pd.DataFrame, keep_target: bool = True) -> pd.DataFrame:
        ...
        if missing_cols:
            logger.warning(
                f"⚠️  {len(missing_cols)} sütun DataFrame'de bulunamadı, atlanıyor: "
                f"{missing_cols[:3]}"
            )
...
def select_features_train_val_test(
    ...
    selected_features, selection_info = selector.select_features(
        train_df,
        method=method,
        n_features=n_features
    )
