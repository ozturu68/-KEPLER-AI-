...
class ExoplanetFeatureEngineer:
    ...
    def __init__(self, poly_degree: int = 2, interaction_only: bool = False, include_bias: bool = False):
        ...
        logger.info(
            f"ExoplanetFeatureEngineer oluşturuldu: " f"degree={poly_degree}, interaction_only={interaction_only}"
        )
    ...
    def create_polynomial_features(
        ...
        self.poly_transformer = PolynomialFeatures(
            degree=self.poly_degree, interaction_only=self.interaction_only, include_bias=self.include_bias
        )
    ...
    def create_interaction_features(
        self, df: pd.DataFrame, feature_pairs: list[tuple[str, str]] | None = None
    ) -> pd.DataFrame:
        """
    ...
    def fit_transform(
        ...
        poly_feature_cols: list[str] | None = None,
    ) -> pd.DataFrame:
        """
        ...
        if create_polynomial:
            df_engineered, poly_names = self.create_polynomial_features(df_engineered, feature_cols=poly_feature_cols)
...
def engineer_train_val_test(
    train_df: pd.DataFrame, val_df: pd.DataFrame, test_df: pd.DataFrame, **kwargs
