...
from src.core import DROP_COLUMNS, OUTLIER_IQR_MULTIPLIER, TARGET_COLUMN, DataValidationError
...
def detect_outliers_iqr(data: pd.Series, multiplier: float = OUTLIER_IQR_MULTIPLIER) -> tuple[pd.Series, int]:
    """
...
def handle_outliers(
    df: pd.DataFrame, columns: list[str], method: str = "clip", multiplier: float = OUTLIER_IQR_MULTIPLIER
) -> pd.DataFrame:
    """
    Outlier'ları işle.
...
def convert_dtypes(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"Memory kullanımı: {initial_memory:.2f}MB → {final_memory:.2f}MB " f"({memory_reduction:.1f}% azalma)")
...
def validate_target_column(df: pd.DataFrame) -> None:
    ...
    if nan_count > 0:
        raise DataValidationError(f"Target sütununda {nan_count} NaN değer var - kaldırılmalı")
