...
class MissingValueHandler:
    ...
    def fit(self, df: pd.DataFrame) -> 'MissingValueHandler':
        ...
        if numerical_cols:
            ...
            logger.info(f"Numerical imputer fit edildi: {len(numerical_cols)} sütun, strategy={self.numerical_strategy}")
        ...
        if categorical_cols:
            ...
            logger.info(f"Categorical imputer fit edildi: {len(categorical_cols)} sütun, strategy={self.categorical_strategy}")
...
def analyze_missing_values(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(missing_df) > 0:
        logger.info(f"En çok missing olan sütun: {missing_df.iloc[0]['column']} (%{missing_df.iloc[0]['missing_pct']:.1f})")
